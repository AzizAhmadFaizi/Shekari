...
<div class="card-header">
    <h4 class="card-title">لست {{ $organization->type == 1 ? 'روسا' : 'مسعولین' }}</h4>
    @if (auth()->user()->role_id == 2)
        <button class="btn btn-primary show_modal" id="new_president_btn"><span
                data-feather='plus'></span>{{ $organization->type == 1 ? 'ریس' : 'مسعول' }} جدید</button>
    @endif
</div>
<div class="card-body">
                ...
                @foreach ($presidents as $item)
                    <tr>
                        ...
                        <th class="text-center">
                            <button type="button" class="btn btn-outline-primary btn-sm president_view_btn round"
                                ...
                                data_last_family_contact_no="{{ $item->last_family_contact_no }}"
                                ...
                                data_img="{{ URL::asset('storage/president_images/' . $item->image) }}"
                                data_status_reason="{{ $item->status_reason }}"><span
                                    class="fa fa-eye"></span></button>
                            @if ($item->is_approved != 1 && auth()->user()->role_id == 2)
                                <button type="button" class="btn btn-outline-dark btn-sm round president_edit_btn"
                                    data-id="{{ $item->id }}" data_name_dr="{{ $item->name_dr }}"
                                    data_grand_father_name="{{ $item->grand_father_name }}"
                                    data_last_name_dr="{{ $item->last_name_dr }}"
                                    data_last_family_contact_no="{{ $item->last_family_contact_no }}"
                                    ...
                                    data_current_district_id="{{ $item->current_district_id }}"
                                    data_current_village="{{ $item->current_village }}"
                                    data_img="{{ URL::asset('storage/president_images/' . $item->image) }}"><span
                                        class="fa fa-edit"></span></button>
                            @endif
                            @if ($item->status == 1)
                                <button type="button" class="btn btn-outline-danger btn-sm round president_cancel_btn"
                                    data-id="{{ $item->id }}"><span class="fa fa-trash"></span></button>
                            @endif
                            @if ($item->is_approved == 0)
                                {{-- <a href='#' class='me-1 president_approve_btn' style='cursor: pointer;' data-id="{{ $item->id }}"><span class='btn btn-outline-success btn-sm waves-effect round fa fa-check'></span></a> --}}
                                {{-- <a href='#' class='me-1 president_reject_btn' style='cursor: pointer;' data-id="{{ $item->id }}"><span class='btn btn-outline-primary btn-sm waves-effect round fa fa-times'></span></a> --}}
                            @endif
    ...
    <div class="modal-dialog form-block">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">دلیل تبدیلی رییس</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('president-deactive') }}" id="predint_cancel_form" method="post">
                @csrf
                <div class="modal-body">
                    <div class="mb-1">
                        <input type="hidden" name="id">
                        <label class="mb-1">دلیل تبدیلی رییس</label>
                        <textarea class="form-control" name="status_reason" id="" cols="30" rows="5"
                            placeholder="دلیل تبدیلی رییس را بنوسید"></textarea>
...
<div class="modal modal-slide-in fade pop_up_modal" id="modals-slide-in">
    <div class="modal-dialog sidebar-xxl form-block">
        <form class="add-new-record modal-content pt-0" id="president_store_form" method="POST"
            action="{{ route('president-store') }}">
            @csrf
            <input type="hidden" name="organization_id" value="{{ $organization_id }}" />
            <input type="hidden" name="organization_type" value="{{ $organization->type }}" />
            <input type="hidden" name="president_id" />
            <div class="modal-header mb-1">
                <h5 class="modal-title" id="presidentModalLabel"></h5>
            </div>
            <div class="modal-body flex-grow-1">
                <div class="row">
                    ...
                    <div class="mb-1 col-sm-12 {{ $organization->type == 1 ? 'col-md-2' : 'col-md-4' }}">
...
<script>
    $(document).on('click', '.president_approve_btn', function() {
        Swal.fire({
            title: "تایید شود؟",
            icon: 'question',
            ...
            if (result.value == true) {
                $.get("{{ route('president-approve') }}/" + $(this).attr('data-id'), function(data) {
                    if (data) {
    ...
    $(document).on('click', '.president_reject_btn', function() {
        Swal.fire({
            title: "رد شود ؟",
            icon: 'question',
            showCancelButton: true,
            ...
            if (result.value == true) {
                $.get("{{ route('president-reject') }}/" + $(this).attr('data-id'), function(data) {
                    if (data) {
    ...
    $(document).on('click', '.president_edit_btn', function(e) {
        $('#presidentModalLabel').html('تغییر مشخصات رییس');
        $('input').prop('disabled', false);
        $('select').prop('disabled', false);
        $('.to_hide').removeClass('d-none');
        $('input[name=president_id]').val($(this).attr('data-id'));
        ...
        $('input[name=email]').val($(this).attr('data_email'));
    ...
    $(document).on('click', '.president_view_btn', function(e) {
        $('#presidentModalLabel').html('نمایش مشخصات رییس');
        $('input[name=president_id]').val($(this).attr('data-id'));
        $('input[name=name_dr]').val($(this).attr('data_name_dr'));
        $('input[name=name_en]').val($(this).attr('data_name_en'));
        $('input[name=last_name_dr]').val($(this).attr('data_last_name_dr'));
        $('input[name=last_name_en]').val($(this).attr('data_last_name_en'));
        $('input[name=father_name]').val($(this).attr('data_father_name'));
        $('input[name=email]').val($(this).attr('data_email'));
        $('input[name=contact_no]').val($(this).attr('data_contact_no'));
        $('input[name=family_contact_no]').val($(this).attr('data_family_contact_no'));
    ...
    $(document).on('click', '.president_cancel_btn', function() {
        $('input[name=id]').val($(this).attr('data-id'));
        $('#cancelModal').modal('show');
    });

    var submit_btn = false;
    $(document).on('submit', '#president_store_form', function(event) {
        event.preventDefault();
        if (!submit_btn) {
            data = new FormData($("#president_store_form")[0]);
            $.ajax({
    ...
    $(document).on('submit', '#predint_cancel_form', function(event) {
        event.preventDefault();
        if (!submit_btn_1) {
            data = new FormData($("#predint_cancel_form")[0]);
            $.ajax({
                url: $(this).attr('action'),
                type: 'post',
                data: data,
